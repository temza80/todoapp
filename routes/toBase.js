...
var params={title: title,teaser:post.substring(0,70), post: post};
...
var post_id=req.body.post_id.toString()
  ...
  Post.findById(post_id, function (err, page) {if(err) {  console.log('loger'+err.message); res.send("Error"); return next(err);}
        ...
        page.save(function(err){if(err) {res.send("Error"); console.log(err); }
			else res.send("OK");
...
module.exports.delPost=function(req,res,next) {
    ...
    Post.remove({_id :req.params.page}, function (err, page) {if(err) {  console.log('loger'+err.message); return next(err);}
